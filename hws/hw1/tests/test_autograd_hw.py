...
# 新增__rsub__测试
def test_rsub_forward():
    np.testing.assert_allclose(1.0-ndl.Tensor([0.,1.,2.]).numpy(), np.array([1.,0.,-1.]))

# PowerScalar和Exp因为小数位太多懒得测了，都是调用numpy接口
...
def test_broadcast_to_backward():
    ...
    gradient_check(ndl.broadcast_to, ndl.Tensor(np.random.randn(3, 1, 5)), shape=(1, 3, 1, 5))
    gradient_check(ndl.broadcast_to, ndl.Tensor(np.random.randn(3, 1, 5)), shape=(3, 1, 5))

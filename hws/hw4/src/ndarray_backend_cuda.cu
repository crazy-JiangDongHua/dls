...
__device__ size_t get_idx(size_t gid, CudaVec shape, CudaVec strides, size_t offset){
  int32_t idx=offset;
  for(int i=((int)(shape.size))-1;i>=0;i--){
    idx+=((int32_t)gid)%shape.data[i]*strides.data[i];
    gid/=(size_t)(shape.data[i]);
  }
  return idx;
}
...
__global__ void ScalarSetitemKernel(size_t size, scalar_t val, scalar_t* out, CudaVec shape,
                              CudaVec strides, size_t offset){
  size_t gid = blockIdx.x * BASE_THREAD_NUM + threadIdx.x;
  if(gid<size)

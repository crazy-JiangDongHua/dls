...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<uint32_t> shape,
  ...
  for(size_t i=0;i<out->size;i++){
    ...
    for(size_t j=0;j<dim;j++)
      idx+=((int32_t)pos[j])*strides[j];
    out->ptr[i]=a.ptr[idx];
    pos[dim-1]++;
    // 注意不要写j>=0，因为j是size_t类型，所以不会小于0，会无限循环
    // 而且第0维不需要进位
...
void ReduceSum(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  /// BEGIN YOUR SOLUTION
  for(size_t i=0;i<out->size;i++){
    // 必须是0. 不能是0 因为最终返回类型是第三个参数的类型 输入0就是int 导致中间结果错误
    out->ptr[i]=std::accumulate(a.ptr+i*reduce_size, a.ptr+(i+1)*reduce_size, 0.);
  }
  /// END YOUR SOLUTION
}

...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if cnt==1:
            ...
            new_shape[idx]=self.size//(prod(new_shape)*-1)
    ...
    def broadcast_to(self, new_shape):
        ...
        padnum=len(new_shape)-self.ndim
        new_strides=[0,]*padnum+list(self.strides)
        for i,d in enumerate(self.shape):
            assert new_shape[i+padnum]==d or d==1
            if d==1:
              new_strides[i+padnum]=0
    ...
    def __getitem__(self, idxs):
        ...
        for i, sl in enumerate(idxs):
            start, stop, step = sl.start, sl.stop, sl.step
            new_shape[i]=1+(stop-start-1)//step
            new_strides[i]*=step
    ...
    def __matmul__(self, other):
        ...
        assert self.shape[1] == other.shape[0], f"self.shape[1]:{self.shape[1]}, other.shape[0]:{other.shape[0]}"
    ...
    def flip(self, axes):
        ...
        for axis in axes:
            ...
            new_strides[axis]*=-1
        out=NDArray.make(self.shape, tuple(new_strides), self.device, self._handle, new_offset)
        return out.compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        new_shape=tuple([l+r+s for (l, r), s in zip(axes, self.shape)])
        out=full(new_shape, 0.0, dtype=self.dtype, device=self.device)
        access=tuple([slice(l,l+s) for (l, _), s in zip(axes, self.shape)])
        out[access]=self
        return out
        ### END YOUR SOLUTION

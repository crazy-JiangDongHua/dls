...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            if i not in self.u:
                self.u[i]=ndl.init.zeros(*param.shape, device=param.device, dtype=param.dtype, requires_grad=False)
            if param.grad is None:
                continue
            # 中间变量传递过程中可能类型提升flaot64，例如在SoftmaxLoss中
            new_grad=ndl.Tensor(param.grad, device=param.device, dtype=param.dtype, requires_grad=False)+self.weight_decay*param.data
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            if i not in self.m:
                self.m[i]=ndl.init.zeros(*param.shape, device=param.device, dtype=param.dtype, requires_grad=False)
            if i not in self.v:
                self.v[i]=ndl.init.zeros(*param.shape, device=param.device, dtype=param.dtype, requires_grad=False)
            if param.grad is None:
                continue
            # 中间变量传递过程中可能类型提升flaot64，例如在SoftmaxLoss中
            new_grad=ndl.Tensor(param.grad, device=param.device, dtype=param.dtype, requires_grad=False)+self.weight_decay*param.data
